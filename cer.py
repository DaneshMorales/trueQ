...
def effective_first_dressed_gate(hard_gate: tq.Gate, easy_gate: tq.Gate, noisy_gates: dict):
    ...
    if hard_gate == Gate.h:
        for p1 in paulis:
            gate = p1@noisy_gates[hard_gate]@noisy_gates[correction_gate(hard_gate,p1)@easy_gate]
            lst.append(tqm.Superop.from_unitary(tq.Simulator().operator(circuit=tq.Circuit([{0: gate}])).mat()).ptm)    
    ...
    if hard_gate == Gate.cnot:
        for (p1,p2) in two_q_paulis.keys():
            gate = two_q_paulis[(p1,p2)]@noisy_gates[hard_gate]@noisy_gates[correction_gate(hard_gate,two_q_paulis[(p1,p2)])@easy_gate]
            lst.append(tqm.Superop.from_unitary(tq.Simulator().operator(circuit=tq.Circuit([{0: gate}])).mat()).ptm)
...
def effective_dressed_gate(hard_gate: tq.Gate, easy_gate: tq.Gate, noisy_gates: dict):
    ...
    if hard_gate == Gate.h:
        for p1 in paulis:
            for p2 in paulis:
                gate = p1@noisy_gates[hard_gate]@noisy_gates[correction_gate(hard_gate,p1)@easy_gate@p2]@p2.adj
                lst.append(tqm.Superop.from_unitary(tq.Simulator().operator(circuit=tq.Circuit([{0: gate}])).mat()).ptm)    
    ...
    if hard_gate == Gate.cnot:
        for (p1,p2) in two_q_paulis.keys():
            for (p3,p4) in two_q_paulis.keys():
                gate = two_q_paulis[(p1,p2)]@noisy_gates[hard_gate]@noisy_gates[correction_gate(hard_gate,two_q_paulis[(p1,p2)])@easy_gate@two_q_paulis[(p3,p4)]]@two_q_paulis[(p3,p4)].adj
                lst.append(tqm.Superop.from_unitary(tq.Simulator().operator(circuit=tq.Circuit([{0: gate}])).mat()).ptm)
...
def effective_last_dressed_gate(easy_gate: tq.Gate, noisy_gates: dict):
    ...
    for p1 in paulis:
        gate = noisy_gates[easy_gate @ p1]@p1.adj
        lst.append(tqm.Superop.from_unitary(tq.Simulator().operator(circuit=tq.Circuit([{0: gate}])).mat()).ptm)    
    return tqm.Superop.from_ptm(np.mean(lst, axis=0)).ptm
